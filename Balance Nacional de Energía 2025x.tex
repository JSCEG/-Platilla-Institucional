...
\hypersetup{
  ...
  pdfsubject={BNE 2025},
  pdfkeywords={Energía},
  pdfcreationdate={D:20251219222701}
}
...
\setDocumentoCorto{Balance Nacional de Energía 2025x}
...
% Referencia a tabla: TBL-2.1.1-2
...
\end{tabladoradoCorto}
\vspace{-4pt}
\fuente{Elaboración de SENER con información del Sistema de Información de Hidrocarburos, Rondas México y Portal de Asignaciones de la SENER * Incluye aguas someras y profundas}
...
% Referencia a tabla: TBL-2.1.2-1
...
En México operan nueve CPG administrados por PEMEX, los cuales cuentan con una capacidad instalada de 4,283 millones de pies cúbicos diarios (MMpcd) para el endulzamiento\footnote{Proceso de eliminación de impurezas como ácido sulfhídrico (H₂S) y dióxido de carbono (CO₂) del gas natural.} de gas, 135 MMpcd para el tratamiento de líquidos, 5,912 MMpcd para procesos criogénicos, y 560 mil barriles diarios (Mbd) para el fraccionamiento de líquidos (Tabla 2.4.).

% Referencia a tabla: TBL-2.1.2-2
...
% Referencia a tabla: TBL-2.1.2-3
...
% Referencia a tabla: TBL-2.1.2-4
\begin{tabladoradoCorto}
...
\begin{tabladoradoCorto}
  \caption{Tabla 2.4 Capacidad instalada de CPG, 2024}
  \label{tab:tabla_2_4_capacidad_instalada_}
...
\begin{tabladoradoCorto}
  \caption{Tabla 2.5 Capacidad instalada por CPQ, 2024}
  \label{tab:tabla_2_5_capacidad_instalada_}
  \begin{tabularx}{\textwidth}{Vvv}
    \toprule
...
\begin{tabladoradoCorto}
  \caption{Tabla 2.6 Capacidad nominal por producto de cada CPQ, 2024}
  \label{tab:tabla_2_6_capacidad_nominal_po}
  \begin{tabularx}{\textwidth}{Vvvvv}
    \toprule
...
% Referencia a tabla: TBL-2.1.3-1
...
% Referencia a tabla: TBL-2.1.3-2
...
% Referencia a tabla: TBL-2.1.3-3
...
% Referencia a tabla: TBL-2.1.3-4
...
% Referencia a tabla: TBL-2.1.3-5
\begin{tabladoradoCorto}
  \caption{Tabla 2.7 Sistemas de transporte de gas seco, 2024}
  \label{tab:tabla_2_7_sistemas_de_transpor}
  \begin{tabularx}{\textwidth}{Vvv}
    \toprule
...
\begin{tabladoradoCorto}
  ...
  \label{tab:tabla_2_8_terminales_de_almace}
...
\begin{tabladoradoCorto}
  \caption{Tabla 2.9. Capacidad de transporte por ducto de petrolíferos y GLP, 2024}
  \label{tab:tabla_2_9__capacidad_de_transp}
  \begin{tabularx}{\textwidth}{Vvv}
    \toprule
...
\end{tabladoradoCorto}
\vspace{-4pt}
\fuente{Elaboración de SENER con información de Infraestructura Nacional de Almacenamiento y Transporte por Ducto de Petrolíferos y Prontuario Estadístico de la SENER. 1 Infraestructura Nacional de Almacenamiento y Transporte por Ducto de Petrolíferos 2024 de la SENER. 2 Prontuario Estadístico de la SENER.}
...
\end{tabladoradoCorto}
\vspace{-4pt}
\fuente{Elaboración de SENER con información del Prontuario Estadístico de la SENER. * Los Distribuidores son por medios distintos de ductos. ASA: Aeropuertos y Servicios Auxiliares.GAFSACOMM: Grupo Aeroportuario, Ferroviario, de Servicios Auxiliares y Conexos Olmeca-Maya-Mexica.}

\begin{tabladoradoCorto}
  \caption{Tabla 2.11. Capacidad de almacenamiento y distribución de GLP, 2024}
  \label{tab:tabla_2_11__capacidad_de_almac}
  \begin{tabularx}{\textwidth}{Vvv}
    \toprule
...
% Referencia a figura: FIG-2.2.1-1
...
% Referencia a tabla: TBL-2.2.1-1
\begin{figure}[H]
  {\centering
  % Texto alternativo para accesibilidad
...
\begin{tabladoradoCorto}
  ...
  \label{tab:tabla_2_12__capacidad_permisio}
  \begin{tabularx}{\textwidth}{Vv}
    \toprule
...
% Referencia a tabla: TBL-2.2.2-1
...
% Referencia a tabla: TBL-2.2.2-2
...
% Referencia a tabla: TBL-2.2.2-3
\begin{tabladoradoCorto}
  \caption{Tabla 2.13. Longitud de las líneas de transmisión de la RNT, 2024}
  \label{tab:tabla_2_13__longitud_de_las_li}
  \begin{tabularx}{\textwidth}{Vv}
    \toprule
...
\begin{tabladoradoCorto}
  \caption{Tabla 2.14. Capacidad de transformación en la RNT, 2024}
  \label{tab:tabla_2_14__capacidad_de_trans}
  \begin{tabularx}{\textwidth}{Vv}
    \toprule
...
\begin{tabladoradoCorto}
  \caption{Tabla 2.15. Características de las líneas de distribución de las RGD, 2024}
  \label{tab:tabla_2_15__caracteristicas_de}
  \begin{tabularx}{\textwidth}{Vv}
    \toprule
    \rowcolor{gobmxDorado} \encabezadodorado{INFRAESTRUCTURA DE DISTRIBUCIÓN} & \encabezadodorado{2,024} \\
    \midrule
...
% Referencia a tabla: TBL-2.2.3-1
...
% Referencia a figura: FIG-2.2.3-1
\begin{figure}[H]
  {\centering
  % Texto alternativo para accesibilidad
...
\begin{tabladoradoCorto}
  \caption{Tabla 2.16. Capacidad Instalada acumulada de GD, 2024}
  \label{tab:tabla_2_16__capacidad_instalad}
  \begin{tabularx}{\textwidth}{Vv}
    \toprule
    \rowcolor{gobmxDorado} \encabezadodorado{Capacidad instalada de Generación distribuida} & \encabezadodorado{} \\
    \midrule
    Total (MW) & 4,447.9 \\
    Contratos (n°) & 518,019 \\
    \bottomrule
...
Destaca que el gas seco, las gasolinas y naftas, y el diésel son imprescindibles para la generación de energía eléctrica, calefacción y la movilidad terrestre de la población y las mercancías. Ello sirve de referente para identificar los energéticos sobre los cuales se deberían enfocar esfuerzos que promuevan la soberanía energética del país.

% Referencia a figura: FIG-3.2-1
\begin{figure}[H]
  {\centering
  % Texto alternativo para accesibilidad
...
La variación de inventarios refiere a la diferencia entre la existencia inicial (1° de enero del año en cuestión) y la existencia final (31 de diciembre del mismo año) de productos energéticos almacenados. Un saldo positivo indica que se ha aumentado el inventario, en contraposición, un saldo negativo significa que los inventarios de energía han disminuido.

En 2024, el valor la variación de inventarios total fue de -14.72 PJ, esto se debió a una ligera reducción de inventarios de energía, lo que repercutió en una disminución en la oferta total, es decir en la energía disponible en el país.

% Referencia a figura: FIG-3.3-1
...
\end{figure}
\vspace{-4pt}
\fuente{Elaboración SENER. * Otros energéticos se compone por energía nuclear, energía hidráulica, energía geotérmica, energía solar, energía eólica, bagazo de caña, leña, biogás, coque de carbón, coque de petróleo, GLP, querosenos, combustóleo, lubricantes, parafinas,asfalto, azufre, materia primanegro de humo, gasóleo, etano, gas de alto horno, gas de coque, licor negro, gas residual y aceite residual, energía eléctrica y carbón mineral.}



\subsection{Energía no aprovechada}

La energía no aprovechada (ENA), se refiere a la energía que, por disponibilidad técnica o económica durante su explotación, no se utiliza. En 2024, la ENA ascendió a 261 PJ, de los cuales el gas natural y el petróleo crudo representaron casi su totalidad, con 206 PJ y 54 PJ, respectivamente, como se observa en la Figura 3.6.

% Referencia a figura: FIG-3.5-1

La ENA en 2024 representó el 3.5\% de la producción nacional de energía. Considerando que el gas natural es el de mayor participación en esta, sigue siendo un tema prioritario aumentar su aprovechamiento, debido a que es el combustible con mayor importación a México, según el saldo neto de la balanza comercial energética. Por ello, algunas líneas de acción del PND y PROSENER 2025-2030 se enfocan en la mejora de la eficiencia energética y el fortalecimiento de las capacidades de producción de las empresas públicas del Estado, a través de la reducción de la quema y venteo de gas natural.
\begin{figure}[H]
  {\centering
  % Texto alternativo para accesibilidad
...
\end{figure}
\vspace{-4pt}
\fuente{Elaboración SENER. * Otros energéticos se compone por lubricantes, parafinas, asfaltos, materia prima negro humo, gasóleo, etano, gas de alto horno, gas de coque, licor negro, gas residual, aceite residual.}
...
\end{figure}
\vspace{-4pt}
\fuente{Elaboración SENER. * En el periodo del 2011-2014, condensados se contabilizan en otros energéticos.}



\subsection{Procesamiento de gas}

En los últimos años, la transformación en plantas de gas y fraccionadoras ha seguido el comportamiento observado en refinerías y despuntadoras. El gas seco es el energético más producido en este tipo de centros de transformación; sin embargo, disminuyó de forma considerable y constante durante el periodo 2013-2020 (Figura 4.3.).

% Referencia a figura: FIG-4.2-1
...
La matriz de insumos energéticos para la generación de energía eléctrica en 2024 estuvo dominada por el gas seco, con el 64.2\% (2186.18 PJ), seguido de fuentes de energía renovable con el 24.8\% (844.13PJ), de los cuales la energía hidráulica representó el 7.7\% (262.18PJ), la energía eólica el 6.5\% (222.18 PJ), la energía solar con el 6\% (205.70 PJ), la geoenergía con el 2.7\% (91.32 PJ), el bagazo de caña con el 1.8\% (61.5 PJ) y el biogás con apenas el 0.1\% (3.25 PJ). En tanto que otros energéticos como el carbón mineral contribuyó con el 4.3\% (147.70 PJ), la energía nuclear con el 3.8\% (129.51 PJ), el combustóleo con el 1.4\% (47.09 PJ), el diésel con el 1\% (33.43 PJ) y el coque de petróleo con apenas el 0.50\% (17.77 PJ) (Figura 4.5.).

% Referencia a figura: FIG-4.3-1
...
Como se muestra en la Figura 4.7., en el periodo 2010-2024, el consumo propio del sector ha disminuido, presentando un comportamiento acoplado a la tendencia a la baja de la producción de energía en el país.

% Referencia a figura: FIG-4.4-1

En 2024, el total de energía consumida por el sector energético fue de 1,608 PJ. El energético de mayor consumo en los procesos e instalaciones del sector fue el gas seco, que representó el 53.4\% (859.49 PJ), seguido por el gas natural con 37\% (595.69 PJ), cubriendo más del 90\% entre ambos energéticos (Ver Figura 4.8.). El restante 9.5\% en el consumo propio del sector está conformado por energía eléctrica con 4.2\% (67.89 PJ), otros energéticos con 1.4\% (23.04 PJ), combustóleo con 0.7\% (11.25 PJ), coque de petróleo con 0.5\% (7.83 PJ), gasolinas y naftas con 0.3\% (4.15 PJ), GLP con 0.2\% (2.66 PJ y coque de carbón con 0.02\% (0.36PJ).

La predominancia del gas seco se ha mantenido a lo largo de los últimos 15 años (Figura 4.7.), con un pico notable en 2020. Esta situación afecta negativamente el avance hacia la autosuficiencia, la soberanía y la seguridad energética del país, dado que el gas seco es el principal energético que se importa en México.

% Referencia a figura: FIG-4.4-2
\begin{figure}[H]
  {\centering
  % Texto alternativo para accesibilidad
...
En el periodo 2010-2024, las pérdidas energéticas presentaron una tendencia decreciente a lo largo del periodo (tasa anual de -0.9\% de crecimiento), principalmente por la reducción de pérdidas técnicas, alcanzando un mínimo en 2019 (147.28 PJ). A partir de ese año, las pérdidas han experimentado un crecimiento, mayormente provocado por incremento en las pérdidas no técnicas. En 2024, las pérdidas alcanzaron un valor de 168.01 PJ, un 5.03\% mayor que en 2023 (Figura 4.9.).

% Referencia a figura: FIG-4.5-1
...
El consumo energético del sector agropecuario fue de 188.02 PJ en 2024. En la Figura 5.3 se observa que los principales energéticos consumidos fueron el diésel y la energía eléctrica (96.1\% del consumo del sector en conjunto), con un consumo de 126.84 PJ (67.5\%) y 53.88 PJ (28.7\%), respectivamente. El restante se distribuye entre el GLP con 3\% (5.55 PJ), combustóleo con 0.7\% (1.29 PJ), y gas seco con 0.2\% (0.47 PJ).

La importancia del diésel en el consumo de este sector es atribuible al uso de maquinaria en el trabajo de campo (tractores, helicópteros de fumigación, camiones de carga de cultivos y ganado, entre otros). Dada la importancia del diésel para el sector agropecuario, es importante remarcar que es uno de los energéticos secundarios de mayor déficit comercial en el país, después del gas seco y las gasolinas y naftas, como se presentó en la Figura 3.3.
...
Aunque en la mayoría de las ramas industriales tuvo un consumo mayoritariamente de gas seco y energía eléctrica en 2024, se estimó que cerca de la mitad requieren primordialmente de energía eléctrica (212, 213, 314, 315, 332, 333, 335, 336 y Otras actividades), mientras que el resto (ramas 311, 313, 322, 325, 326, 331 y 336) es altamente dependiente del gas seco. También se concluyó que, la fabricación de productos cuya base es de minerales no metálicos (rama 327) tiene una menor dependencia a los dos energéticos preponderantes en el sector y consume primordialmente coque de petróleo; mientras que, de manera similar, la industria alimentaria (rama 213) tiene un importante consumo de bagazo de caña. Finalmente, aunque sus consumos totales no son grandes (<18 PJ), las industrias relacionadas con la construcción (236, 237, 238) hacen uso primordialmente del diésel (Figura 5.8).

% Referencia a figura: FIG-5.2-3
...
% Referencia a figura: FIG-5.4-2

El consumo de energía eléctrica en el sector residencial está relacionado con la iluminación, climatización y al uso de electrodomésticos (refrigeradores, lavadoras, televisores, etc.) . En tanto que para la preparación de alimentos y el calentamiento de agua, son el gas licuado de petróleo y la leña los energéticos más utilizados en las viviendas de México (Figura 5.14.).

En este sentido, la leña es el tercer energético más consumido en el sector (Figura 5.12.), utilizado principalmente en la cocción de alimentos y calentamiento de agua, cuyo usuarios se localizan principalmente en zonas rurales y periurbanas del país. Asimismo, muchos de los usuarios utilizan dispositivos tradicionales que tienen impactos negativos en la salud por sus emisiones intramuros, lo cual afecta principalmente a las mujeres por ser quienes comúnmente realizan las labores de cocción de alimentos. Bajo esta perspectiva, se busca la implementación de dispositivos más eficientes y la expulsión de las emisiones fuera de las viviendas como parte de las políticas y acciones establecidas en el PND 2025-2030.

% Referencia a figura: FIG-5.4-3
\begin{figure}[H]
  {\centering
  % Texto alternativo para accesibilidad
...
El consumo energético del sector público está asociado a los servicios públicos de bombeo de agua potable y el alumbrado público. Estos servicios utilizan únicamente energía eléctrica. En 2024, este sector tuvo un consumo de 26.16 PJ en donde el alumbrado público participó con el 24.1\% (6.31 PJ) y el del bombeo de agua potable con el 75.9\% (19.85 PJ), por lo que representó el sector con el menor consumo energético final en ese año.

El consumo del sector se ha mantenido relativamente constante en los últimos 15 años, así como la proporción de consumo entre usos finales (\textasciitilde{} 75\% bombeo de agua y \textasciitilde{}25\% alumbrado público).

% Referencia a figura: FIG-5.5-1
\begin{figure}[H]
  {\centering
  % Texto alternativo para accesibilidad
...
% Referencia a figura: FIG-5.6-2

En la Figura 5.18. se observan los combustibles utilizados por los subsectores considerados. En el caso del autotransporte, se consumen gasolinas y naftas primordialmente, debido a que se incluyen diversos vehículos ligeros (automóviles, camionetas, taxis, entre otros); seguido del diésel por el transporte de pasajeros y de carga (autobuses, camiones de carga, transporte público, etc.).

En el transporte aéreo únicamente tiene consumo de querosenos, por el uso de turbosina como estándar en la aviación. Mientras que el consumo del subsector ferroviario está concentrado mayoritariamente en el diésel y marginalmente por la energía eléctrica, ya que además de los ferrocarriles para transporte de carga, se incluyen sistemas de transporte colectivo como es el Metro de la Ciudad de México. Finalmente, el subsector marítimo utiliza principalmente diésel y se complementa con combustóleo.

% Referencia a figura: FIG-5.6-3
\begin{figure}[H]
  {\centering
  % Texto alternativo para accesibilidad
...
\end{figure}
\vspace{-4pt}
\fuente{Elaboración SENER. * Otros energéticos contemplan a lubricantes, parafinas, asfaltos, azufre, materia prima negro de humo, gasóleo, etano, gas de alto horno, gas de coque, licor negro, gas residual y aceite residual.}
...
En esta sección, se presenta un panorama general del SEM en 2024, utilizando las bases energéticas y los datos descritos previamente. Mediante figuras y tablas resumen, se analizan los datos clave de la producción, transformación y consumo de energía primaria y secundaria que se dio en el año 2024 para facilitar la comprensión de esta área estratégica para la economía y el bienestar social del país.

En la Figura 6.1, se observa que en las principales cuentas del Balance Nacional de Energía (BNE), la producción de energía e importación componen, en su mayoría, a la oferta total de energía en el país, complementada por la variación de inventarios. Posteriormente, al restar las exportaciones y la energía no aprovechada, se obtiene oferta interna bruta disponible para los procesos de transformación y en algunos casos, hacia el consumo final. Antes de llegar a usuarios finales o intermedios, se consideran las pérdidas y el consumo propio del sector energético. Finalmente, se presenta el análisis por cada uno de los sectores de consumo final de energía: agropecuario, industrial, comercial y servicios, residencial, público y transporte, así como la participación del consumo no energético.
...
En la Figura 6.3. se muestran los requerimientos energéticos por tipo de destino de la oferta interna bruta, el 57.7\% correspondió al consumo final, mientras que el \% fue la energía requerida por los centros de transformación, el \% fue para el consumo propio del sector y el \% fueron las pérdidas debido a procesos como transporte, transmisión y distribución de los energéticos.

En lo que respecta al consumo final, en la Figura 6.4. se destaca que los sectores transporte e industrial son los principales consumidores finales de energía en el país, con 44\% y 32.8\% respectivamente, en tanto que el sector residencial consume el 12.7\%, y los demás sectores representan consumos más pequeños como el comercial y servicios, agropecuario con 3.4\% cada uno, así también el consumo no energético representa el 3.3\% y el de menor consumo es el público con el 0.5\%.

En la Figura 6.6. se muestra este mismo consumo final a nivel nacional pero desagregado por energético, y consecuente con el consumo sectorial, los energéticos más consumidos en 2024 fueron las gasolinas con 28.8\%, la energía eléctrica con 20.7\%, el gas seco y el diésel con 14.9\% y 13.9\%, respectivamente, así como el GLP con el 8.6\%. El restante 9.7\% se distribuye en los demás energéticos.
...
Finalmente, la Figura 6.7 integra los principales flujos del Balance Nacional de Energía para 2024. Este flujograma cuantitativo muestra las dinámicas entre energéticos, sectores y procesos desde la producción hasta el consumo final de la energía.


\subsection{Principales cuentas del Balance Nacional de Energía}

% Referencia a figura: FIG-6.1-1

% Referencia a figura: FIG-6.1-2

% Referencia a figura: FIG-6.1-3

% Referencia a figura: FIG-6.1-4

% Referencia a figura: FIG-6.1-5
\begin{figure}[H]
  {\centering
  % Texto alternativo para accesibilidad
...
% Referencia a tabla: TBL-6.2-1

% Referencia a tabla: TBL-6.2-2

% Referencia a tabla: TBL-6.2-3

% Referencia a tabla: TBL-6.2-4
\begin{tabladoradoLargo}
  \setlength{\SENERLongTableFirstColWidth}{0.34\textwidth}
  \begin{xltabular}{\textwidth}{QZZZZZZZZZZZZZ}
    ...
    Oferta interna bruta & \SENERNum{219.46} & \SENERNum{1,747.67} & \SENERNum{636.74} & \SENERNum{1,648.09} & \SENERNum{129.51} & \SENERNum{262.18} & \SENERNum{91.32} & \SENERNum{252.18} & \SENERNum{220.18} & \SENERNum{94.72} & \SENERNum{129.42} & \SENERNum{3.25} & \SENERNum{5,434.71} \\
    \hspace{1.35em} Oferta total & \SENERNum{219.56} & \SENERNum{3,674.19} & \SENERNum{637.14} & \SENERNum{1,853.93} & \SENERNum{129.51} & \SENERNum{262.18} & \SENERNum{91.32} & \SENERNum{252.18} & \SENERNum{220.18} & \SENERNum{95.77} & \SENERNum{129.42} & \SENERNum{3.25} & \SENERNum{7,566.61} \\
    \hspace{2.7em} Producción & \SENERNum{14.99} & \SENERNum{3,654} & \SENERNum{638} & \SENERNum{1,868} & \SENERNum{129.51} & \SENERNum{262.18} & \SENERNum{91.32} & \SENERNum{252.18} & \SENERNum{220.18} & \SENERNum{95.77} & \SENERNum{129.42} & \SENERNum{3.25} & \SENERNum{7,358.78} \\
    \hspace{2.7em} Importación & \SENERNum{206.15} & \SENERNum{13.2} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{219.35} \\
    \hspace{2.7em} Variación de inventarios & \SENERNum{-1.58} & \SENERNum{6.98} & \SENERNum{-0.86} & \SENERNum{-14.07} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-9.52} \\
    \hspace{1.35em} Exportación & \SENERNum{-0.09} & \SENERNum{-1,873} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-1,873.09} \\
    \hspace{1.35em} Energía no aprovechada & \SENERNum{0} & \SENERNum{-53.52} & \SENERNum{-0.4} & \SENERNum{-205.84} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-1.05} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-260.8} \\
    Total transformaciónes & \SENERNum{-147.47} & \SENERNum{-1,793.27} & \SENERNum{-638.37} & \SENERNum{-1,052.4} & \SENERNum{-129.51} & \SENERNum{-262.18} & \SENERNum{-91.32} & \SENERNum{-205.7} & \SENERNum{-220.18} & \SENERNum{-61.5} & \SENERNum{0} & \SENERNum{-3.25} & \SENERNum{-4,605.13} \\
    \hspace{1.35em} Coquizadoras y hornos & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    \hspace{1.35em} Refinerías y despuntadoras & \SENERNum{0} & \SENERNum{-1,793.27} & \SENERNum{-326.21} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-2,119.48} \\
    \hspace{1.35em} Plantas de gas y fraccionadoras & \SENERNum{0} & \SENERNum{0} & \SENERNum{-312.16} & \SENERNum{-1,052.4} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-1,364.56} \\
    \hspace{1.35em} Centrales eléctricas & \SENERNum{-147.47} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-129.51} & \SENERNum{-262.18} & \SENERNum{-91.32} & \SENERNum{-205.7} & \SENERNum{-220.18} & \SENERNum{-61.5} & \SENERNum{0} & \SENERNum{-3.25} & \SENERNum{-1,121.09} \\
    \hspace{2.7em} Carboeléctrica & \SENERNum{-147.47} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-147.47} \\
    \hspace{2.7em} Termica convencional & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-56.67} & \SENERNum{0} & \SENERNum{-0.42} & \SENERNum{-57.08} \\
    \hspace{2.7em} Combustión interna & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-4.83} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-4.83} \\
    \hspace{2.7em} Turbogas & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-2.83} & \SENERNum{-2.83} \\
    \hspace{2.7em} Ciclo combinado & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    \hspace{2.7em} Nucleoeléctrica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-129.51} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-129.51} \\
    \hspace{2.7em} Cogeneración & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    \hspace{2.7em} Hidroeléctrica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-262.18} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-262.18} \\
    \hspace{2.7em} Geotérmica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-91.32} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-91.32} \\
    \hspace{2.7em} Eólica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-220.18} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-220.18} \\
    \hspace{2.7em} Solar fotovoltáica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-205.7} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-205.7} \\
  \end{xltabular}
...
\begin{tabladoradoLargo}
  ...
  \begin{xltabular}{\textwidth}{QZZZZZZZZZZZZZ}
    ...
    Consumo del sector y pérdidas & \SENERNum{0} & \SENERNum{45.6} & \SENERNum{1.63} & \SENERNum{-595.69} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-548.46} \\
    \hspace{1.35em} Consumo propio del sector & \SENERNum{0} & \SENERNum{-12.4} & \SENERNum{0} & \SENERNum{-595.69} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-608.09} \\
    \hspace{1.35em} Diferencia estadística & \SENERNum{0} & \SENERNum{69.82} & \SENERNum{1.63} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{71.45} \\
    \hspace{1.35em} Pérdidas técnicas por transporte, transmisión y distribución & \SENERNum{0} & \SENERNum{-11.82} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-11.82} \\
    \hspace{2.7em} Pérdidas en transporte y transmisión & \SENERNum{0} & \SENERNum{-11.82} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-11.82} \\
    \hspace{2.7em} Pérdidas técnicas en distribución & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    \hspace{2.7em} Pérdidas no técnicas  en distribución & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    Consumo final total & \SENERNum{72} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{46.48} & \SENERNum{0} & \SENERNum{33.22} & \SENERNum{129.42} & \SENERNum{0} & \SENERNum{281.12} \\
    \hspace{1.35em} Consumo final no energético & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0.04} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0.04} \\
    \hspace{2.7em} Petroquímica Pemex & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    \hspace{2.7em} Otras ramas económicas & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0.04} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0.04} \\
    Consumo final energético & \SENERNum{72} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{46.48} & \SENERNum{0} & \SENERNum{33.18} & \SENERNum{129.42} & \SENERNum{0} & \SENERNum{281.08} \\
    \hspace{2.7em} Agropecuario & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    \hspace{2.7em} Industrial & \SENERNum{72} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{31.49} & \SENERNum{0} & \SENERNum{33.18} & \SENERNum{9.82} & \SENERNum{0} & \SENERNum{146.49} \\
    \hspace{2.7em} Comercial y servicios & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{4.41} & \SENERNum{0} & \SENERNum{0} & \SENERNum{4.66} & \SENERNum{0} & \SENERNum{9.07} \\
    \hspace{2.7em} Residencial & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{10.58} & \SENERNum{0} & \SENERNum{0} & \SENERNum{114.93} & \SENERNum{0} & \SENERNum{125.51} \\
    \hspace{2.7em} Público & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    \hspace{2.7em} Transporte & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    Prod. bruta energía secundaria & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
  \end{xltabular}
\end{tabladoradoLargo}
\vspace{-4pt}
\fuente{Elaboración SENER}
...
\begin{tabladoradoLargo}
  \setlength{\SENERLongTableFirstColWidth}{0.34\textwidth}
  \begin{xltabular}{\textwidth}{QZZZZZZZZZZZZ}
    ...
    Oferta interna bruta & \SENERNum{16.82} & \SENERNum{119.25} & \SENERNum{297.83} & \SENERNum{1,073.94} & \SENERNum{127.03} & \SENERNum{506.11} & \SENERNum{-496.67} & \SENERNum{-0.27} & \SENERNum{2,569.96} & \SENERNum{-6.94} & \SENERNum{4,207.06} & \SENERNum{9,641.77} \\
    \hspace{1.35em} Oferta total & \SENERNum{16.82} & \SENERNum{121.84} & \SENERNum{297.83} & \SENERNum{1,078.42} & \SENERNum{127.03} & \SENERNum{540.53} & \SENERNum{0.33} & \SENERNum{-0.27} & \SENERNum{2,578.96} & \SENERNum{23.51} & \SENERNum{4,785} & \SENERNum{12,353.61} \\
    \hspace{2.7em} Producción & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{7,358.78} \\
    \hspace{2.7em} Importación & \SENERNum{16.82} & \SENERNum{118.73} & \SENERNum{298} & \SENERNum{1,084} & \SENERNum{126.76} & \SENERNum{543.51} & \SENERNum{0.39} & \SENERNum{0} & \SENERNum{2,578.47} & \SENERNum{23.51} & \SENERNum{4,790.19} & \SENERNum{5,009.54} \\
    \hspace{2.7em} Variación de inventarios & \SENERNum{0} & \SENERNum{3.11} & \SENERNum{-0.17} & \SENERNum{-5.58} & \SENERNum{0.27} & \SENERNum{-2.98} & \SENERNum{-0.06} & \SENERNum{-0.27} & \SENERNum{0.49} & \SENERNum{0} & \SENERNum{-5.2} & \SENERNum{-14.72} \\
    \hspace{1.35em} Exportación & \SENERNum{0} & \SENERNum{-2.59} & \SENERNum{0} & \SENERNum{-4.48} & \SENERNum{0} & \SENERNum{-34.42} & \SENERNum{-497} & \SENERNum{0} & \SENERNum{-9} & \SENERNum{-30.45} & \SENERNum{-577.94} & \SENERNum{-2,451.03} \\
    \hspace{1.35em} Energía no aprovechada & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-260.8} \\
    Total transformaciones & \SENERNum{0} & \SENERNum{15.05} & \SENERNum{168.92} & \SENERNum{514.64} & \SENERNum{66.33} & \SENERNum{311.03} & \SENERNum{527.88} & \SENERNum{88.9} & \SENERNum{-961.45} & \SENERNum{1,381.52} & \SENERNum{2,112.82} & \SENERNum{-2,492.31} \\
    \hspace{1.35em} Coquizadoras y hornos & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} \\
    \hspace{1.35em} Refinerías y despuntadoras & \SENERNum{0} & \SENERNum{32.82} & \SENERNum{18.55} & \SENERNum{441.01} & \SENERNum{66.33} & \SENERNum{344.46} & \SENERNum{574.97} & \SENERNum{38.88} & \SENERNum{590.63} & \SENERNum{0} & \SENERNum{2,107.65} & \SENERNum{-11.84} \\
    \hspace{1.35em} Plantas de gas y fraccionadoras & \SENERNum{0} & \SENERNum{0} & \SENERNum{150.37} & \SENERNum{73.63} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{50.03} & \SENERNum{634.1} & \SENERNum{0} & \SENERNum{908.12} & \SENERNum{-456.44} \\
    \hspace{1.35em} Centrales eléctricas & \SENERNum{0} & \SENERNum{-17.77} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-33.43} & \SENERNum{-47.09} & \SENERNum{0} & \SENERNum{-2,186.18} & \SENERNum{1,381.52} & \SENERNum{-902.95} & \SENERNum{-2,024.04} \\
    \hspace{2.7em} Carboeléctrica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{50.44} & \SENERNum{50.44} & \SENERNum{-97.02} \\
    \hspace{2.7em} Térmica convencional & \SENERNum{0} & \SENERNum{-17.77} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-46.15} & \SENERNum{0} & \SENERNum{-273.55} & \SENERNum{122.59} & \SENERNum{-214.88} & \SENERNum{-271.96} \\
    \hspace{2.7em} Combustión interna & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-5.12} & \SENERNum{-0.94} & \SENERNum{0} & \SENERNum{-49.06} & \SENERNum{16.91} & \SENERNum{-38.21} & \SENERNum{-43.04} \\
    \hspace{2.7em} Turbogas & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-28.31} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-99.85} & \SENERNum{57.94} & \SENERNum{-70.22} & \SENERNum{-73.05} \\
    \hspace{2.7em} Ciclo combinado & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-1,606.02} & \SENERNum{802} & \SENERNum{-804.02} & \SENERNum{-804.02} \\
    \hspace{2.7em} Nucleoeléctrica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{44.3} & \SENERNum{44.3} & \SENERNum{-85.2} \\
    \hspace{2.7em} Cogeneración & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-157.7} & \SENERNum{46.83} & \SENERNum{-110.87} & \SENERNum{-110.87} \\
    \hspace{2.7em} Hidroeléctrica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{86.52} & \SENERNum{86.52} & \SENERNum{-175.66} \\
    \hspace{2.7em} Geotérmica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{13.43} & \SENERNum{13.43} & \SENERNum{-77.89} \\
    \hspace{2.7em} Eólica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{72.66} & \SENERNum{72.66} & \SENERNum{-147.52} \\
    \hspace{2.7em} Solar fotovoltáica & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{67.88} & \SENERNum{67.88} & \SENERNum{-137.82} \\
  \end{xltabular}
\end{tabladoradoLargo}
\vspace{-4pt}
\fuente{Elaboración SENER}
...
\begin{tabladoradoLargo}
  \setlength{\SENERLongTableFirstColWidth}{0.34\textwidth}
  \begin{xltabular}{\textwidth}{QZZZZZZZZZZZZ}
    \caption{Tabla 6.4. Matriz origen destino, consumo propio, pérdidas y consumo final de energéticos secundarios, 2024}\label{tab:tabla_6_4__matriz_origen_desti}\\
    \toprule
    \rowcolor{gobmxDorado} \encabezadodorado{} & \encabezadodorado{\SENERVHeader{Coque de carbón}} & \encabezadodorado{\SENERVHeader{Coque de petróleo}} & \encabezadodorado{\SENERVHeader{GLP}} & \encabezadodorado{\SENERVHeader{Gasolinas y naftas}} & \encabezadodorado{\SENERVHeader{Querosenos}} & \encabezadodorado{\SENERVHeader{Diesel}} & \encabezadodorado{\SENERVHeader{Combustóleo}} & \encabezadodorado{\SENERVHeader{Otros energéticos}} & \encabezadodorado{\SENERVHeader{Gas seco}} & \encabezadodorado{\SENERVHeader{Energía eléctrica}} & \encabezadodorado{\SENERVHeader{Total energéticos secundarios}} & \encabezadodorado{\SENERVHeader{Total energéticos}} \\
    \midrule
    \endfirsthead
    ...
    Consumo del sector y pérdidas & \SENERNum{-0.36} & \SENERNum{-5.65} & \SENERNum{13.17} & \SENERNum{14.94} & \SENERNum{-0.04} & \SENERNum{-46.5} & \SENERNum{-14.9} & \SENERNum{7.1} & \SENERNum{-780.89} & \SENERNum{-224.08} & \SENERNum{-1,037.21} & \SENERNum{-1,585.67} \\
    \hspace{1.35em} Consumo propio del sector & \SENERNum{-0.36} & \SENERNum{-7.83} & \SENERNum{-2.66} & \SENERNum{-4.15} & \SENERNum{0} & \SENERNum{-23.94} & \SENERNum{-11.25} & \SENERNum{-23.04} & \SENERNum{-859.49} & \SENERNum{-67.89} & \SENERNum{-1,000.6} & \SENERNum{-1,608.69} \\
    \hspace{1.35em} Diferencia estadística & \SENERNum{0} & \SENERNum{2.18} & \SENERNum{15.83} & \SENERNum{19.09} & \SENERNum{-0.04} & \SENERNum{-22.56} & \SENERNum{-3.65} & \SENERNum{30.14} & \SENERNum{78.6} & \SENERNum{0} & \SENERNum{119.58} & \SENERNum{191.03} \\
    \hspace{1.35em} Pérdidas técnicas por transporte, transmisión y distribución & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-156.19} & \SENERNum{-156.19} & \SENERNum{-168.01} \\
    \hspace{2.7em} Pérdidas en transporte y transmisión & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-30.68} & \SENERNum{-30.68} & \SENERNum{-42.5} \\
    \hspace{2.7em} Pérdidas técnicas en distribución & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-60.38} & \SENERNum{-60.38} & \SENERNum{-60.38} \\
    \hspace{2.7em} Pérdidas no técnicas  en distribución & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{-65.13} & \SENERNum{-65.13} & \SENERNum{-65.13} \\
    Consumo final total & \SENERNum{16.46} & \SENERNum{128.65} & \SENERNum{479.92} & \SENERNum{1,603.52} & \SENERNum{193.31} & \SENERNum{770.64} & \SENERNum{16.3} & \SENERNum{95.74} & \SENERNum{827.62} & \SENERNum{1,150.5} & \SENERNum{5,282.66} & \SENERNum{5,563.78} \\
    \hspace{1.35em} Consumo final no energético & \SENERNum{0} & \SENERNum{0} & \SENERNum{0.46} & \SENERNum{10.69} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{95.74} & \SENERNum{76.25} & \SENERNum{0} & \SENERNum{183.14} & \SENERNum{183.18} \\
    \hspace{2.7em} Petroquímica Pemex & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{10.69} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{76.25} & \SENERNum{0} & \SENERNum{86.94} & \SENERNum{86.94} \\
    \hspace{2.7em} Otras ramas económicas & \SENERNum{0} & \SENERNum{0} & \SENERNum{0.46} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{95.74} & \SENERNum{0} & \SENERNum{0} & \SENERNum{96.2} & \SENERNum{96.24} \\
    Consumo final energético & \SENERNum{16.46} & \SENERNum{128.65} & \SENERNum{479.46} & \SENERNum{1,592.83} & \SENERNum{193.31} & \SENERNum{770.64} & \SENERNum{16.3} & \SENERNum{0} & \SENERNum{751.37} & \SENERNum{1,150.5} & \SENERNum{5,099.52} & \SENERNum{5,380.6} \\
    \hspace{2.7em} Agropecuario & \SENERNum{0} & \SENERNum{0} & \SENERNum{5.55} & \SENERNum{0} & \SENERNum{0} & \SENERNum{126.84} & \SENERNum{1.29} & \SENERNum{0} & \SENERNum{0.47} & \SENERNum{53.88} & \SENERNum{188.02} & \SENERNum{188.02} \\
    \hspace{2.7em} Industrial & \SENERNum{16.46} & \SENERNum{128.65} & \SENERNum{52.99} & \SENERNum{1.01} & \SENERNum{0} & \SENERNum{63.34} & \SENERNum{10.98} & \SENERNum{0} & \SENERNum{701.91} & \SENERNum{705.12} & \SENERNum{1,680.46} & \SENERNum{1,826.95} \\
    \hspace{2.7em} Comercial y servicios & \SENERNum{0} & \SENERNum{0} & \SENERNum{82.07} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{18.01} & \SENERNum{78.65} & \SENERNum{178.73} & \SENERNum{187.8} \\
    \hspace{2.7em} Residencial & \SENERNum{0} & \SENERNum{0} & \SENERNum{268.09} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{29.46} & \SENERNum{281.23} & \SENERNum{578.77} & \SENERNum{704.29} \\
    \hspace{2.7em} Público & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{0} & \SENERNum{26.16} & \SENERNum{26.16} & \SENERNum{26.16} \\
    \hspace{2.7em} Transporte & \SENERNum{0} & \SENERNum{0} & \SENERNum{70.76} & \SENERNum{1,591.82} & \SENERNum{193.31} & \SENERNum{580.46} & \SENERNum{4.03} & \SENERNum{0} & \SENERNum{1.53} & \SENERNum{5.46} & \SENERNum{2,447.37} & \SENERNum{2,447.37} \\
    Prod. bruta energía secundaria & \SENERNum{0} & \SENERNum{32.82} & \SENERNum{168.92} & \SENERNum{514.64} & \SENERNum{66.33} & \SENERNum{344.46} & \SENERNum{574.97} & \SENERNum{88.9} & \SENERNum{1,224.73} & \SENERNum{1,381.52} & \SENERNum{4,397.29} & \SENERNum{4,397.29} \\
  \end{xltabular}
\end{tabladoradoLargo}
\vspace{-4pt}
\fuente{Elaboración SENER}
